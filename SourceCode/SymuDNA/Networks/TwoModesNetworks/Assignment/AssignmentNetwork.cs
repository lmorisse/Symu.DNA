...
namespace Symu.DNA.Networks.TwoModesNetworks.Assignment
{
    ...
    public class AssignmentNetwork
    {
        ...
        /// <returns></returns>
        public Matrix<float> ToMatrix(VectorNetwork<IAgentId> agentIds, VectorNetwork<IId> activityIds)
        {
            if (!agentIds.Any || !activityIds.Any)
            {
                return null;
            }
            var matrix = Matrix<float>.Build.Dense(agentIds.Count, activityIds.Count);
            for (var i = 0; i < agentIds.Count; i++)
            {
                ...
                foreach (var agentActivity in GetAgentActivities(agentId))
                {
                    if (!activityIds.ItemIndex.ContainsKey(agentActivity.Id))
                    {
                        throw new NullReferenceException(nameof(activityIds.ItemIndex));
                    }
                    var column = activityIds.ItemIndex[agentActivity.Id];
