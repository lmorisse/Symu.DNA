...
namespace Symu.DNA.Networks.TwoModesNetworks
{
    ...
    public class AgentBeliefNetwork
    {
        ...
        public void Add(IAgentId agentId, AgentBeliefs beliefs)
        {
            if (beliefs is null)
            {
            ...
            AddAgentId(agentId, beliefs);


            foreach (var agentBelief in beliefs.List.Where(a => !List[agentId].Contains(a)))
            {
                List[agentId].Add(agentBelief);
            }
        }
        ...
        public void AddAgentId(IAgentId agentId, AgentBeliefs agentBeliefs)
        {
            if (!Exists(agentId))
            {
                List.TryAdd(agentId, agentBeliefs);
            }
        }
        ...
        public void CopyTo(AgentBeliefNetwork network)
        {
            if (network is null)
            {
            ...
            {
                AgentBeliefs agentBeliefs = new AgentBeliefs { List = keyValuePair.Value.List.ToList() };

                network.Add(keyValuePair.Key, agentBeliefs);
            }
        }
    }
}